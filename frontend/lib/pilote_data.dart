...
class PiloteSignalPainter extends CustomPainter {
  final List<double> signal;

  PiloteSignalPainter(this.signal);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < signal.length; i++) {
      final x = (i / (signal.length - 1)) * size.width;
      final y = size.height - ((signal[i] - minVal) / range) * size.height;
...
class PiloteSignalViewerPage extends StatelessWidget {
  ...
  List<double>? _extractSignal(SessionDataManager manager) {
  ...
  Widget build(BuildContext context) {
    ...
    final signal = _extractSignal(manager);
                      ...
                      child: ClipRRect(
                        ...
                        child: LayoutBuilder(
                          builder: (context, constraints) {
                            return CustomPaint(
                              painter: PiloteSignalPainter(signal),
